...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['buid']) && isset($_POST['ouid']) && isset($_POST['status']) && isset($_POST['disease']) && isset($_POST['stage']) && isset($_POST['medicines']) && isset($_POST['family']) && isset($_POST['findings'])) {
        ...
        if (isset($_POST['treated']) && !empty($_POST['treated'])) {
            ...
            $sqlStatement = "INSERT INTO hms_project_database.medical (BUID, Disease, OUID, HID, Status, Stage, Medicines, Family, Findings, Treated, Diagnosed, Other) VALUES ($buid, '$disease', $ouid, $hid, '$status', '$stage', '$medicines', '$family', '$findings', '$treated', NULL, NULL);";
        }

        if (isset($_POST['diagnosed']) && !empty($_POST['diagnosed'])) {
            $diagnosed = test_input($_POST['diagnosed']);
            $sqlStatement = "INSERT INTO hms_project_database.medical (BUID, Disease, OUID, HID, Status, Stage, Medicines, Family, Findings, Treated, Diagnosed, Other) VALUES ($buid, '$disease', $ouid, $hid, '$status', '$stage', '$medicines', '$family', '$findings', NULL, '$diagnosed', NULL);";
        }
        ...
        if (isset($_POST['other']) && !empty($_POST['other'])) {
            ...
            if (isset($_POST['treated']) && !empty($_POST['treated'])) {
                $treated = test_input($_POST['treated']);
                $sqlStatement = "INSERT INTO hms_project_database.medical (BUID, Disease, OUID, HID, Status, Stage, Medicines, Family, Findings, Treated, Diagnosed, Other) VALUES ($buid, '$disease', $ouid, $hid, '$status', '$stage', '$medicines', '$family', '$findings', '$treated', NULL, '$other');";
            }
    
            elseif (isset($_POST['diagnosed']) && !empty($_POST['diagnosed'])) {
                $diagnosed = test_input($_POST['diagnosed']);
                $sqlStatement = "INSERT INTO hms_project_database.medical (BUID, Disease, OUID, HID, Status, Stage, Medicines, Family, Findings, Treated, Diagnosed, Other) VALUES ($buid, '$disease', $ouid, $hid, '$status', '$stage', '$medicines', '$family', '$findings', NULL, '$diagnosed', '$other');";
            }

            else {
                $sqlStatement = "INSERT INTO hms_project_database.medical (BUID, Disease, OUID, HID, Status, Stage, Medicines, Family, Findings, Treated, Diagnosed, Other) VALUES ($buid, '$disease', $ouid, $hid, '$status', '$stage', '$medicines', '$family', '$findings', NULL, NULL, '$other');";
            }
        }
