...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['buid']) && isset($_POST['ouid']) && isset($_POST['remarks']) && isset($_POST['followup']) && isset($_POST['allergies']) && isset($_POST['height']) && isset($_POST['weight']) && isset($_POST['medicines'])) {
        ...
        $tempSql = "SELECT AllergiesReactions FROM hms_project_database.clientinfo WHERE UID = $buid;";
        ...
        $sql1 = "INSERT INTO hms_project_database.prescription (BUID, OUID, HID, Remarks, FollowUp, Medicines) VALUES ($buid, '$ouid', '$hid', '$remarks', '$followup', '$medicines');";
        $sql2 = "UPDATE hms_project_database.clientinfo SET AllergiesReactions = '$new_allergies', Height = $height, Weight = $weight, BMI = $bmi WHERE UID = $buid;";
            ...
            <div class="form-container" style="padding-left: 10px;">
                <form method="post" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>">
                    ...
                    <div class="form-field">
                        ...
                        <input type="number" step="0.01" id="height" name="height" required="" maxlength="4"
                            oninput="javascript: if (this.value.length > this.maxLength) this.value = this.value.slice(0, this.maxLength);">
                    </div>

                    <div class="form-field">
                        <label for="weight">Weight (in Kilograms) :</label>
                        <input type="number" id="weight" name="weight" required="" maxlength="3"
                            oninput="javascript: if (this.value.length > this.maxLength) this.value = this.value.slice(0, this.maxLength);">
                    </div>
                    ...
                    <center>
                        ...
                        <a href="./M_HealthInfo.php"><button class="btn btn-success" type="submit" style="margin-left: 10px;">Go back</button></a>
                    </center>
                </form>
            </div>
        </section>
