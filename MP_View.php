...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['buid']) && isset($_POST['hid']) && isset($_POST['dateofprescription'])) {

        $buid = test_input($_POST['buid']);
        $hid = test_input($_POST['hid']);
        $specificdate = test_input($_POST['dateofprescription']);
        if ($specificdate == "") {
            $sql = "SELECT * FROM hms_project_database.prescription WHERE BUID = $buid AND HID = $hid;";
        } else {
            $sql = "SELECT * FROM hms_project_database.prescription WHERE BUID = $buid AND CurrentDate = '$specificdate' AND HID = $hid;";
        }
            ...
            <form method="post" action="<?php echo htmlspecialchars($_SERVER[" PHP_SELF"]); ?>">
                <div class="form-container">
                    ...
                    <div class="form-field">
                        ...
                        <input type="number" required="" id="HID" name="hid" maxlength="5"
                            oninput="javascript: if (this.value.length > this.maxLength) this.value = this.value.slice(0, this.maxLength);"
                            value="<?php echo $_POST['HID'] ?? ''; ?>">
                    </div>

                    <div class="form-field">
                        <label for="followUpDate">Date of prescription:</label>
                        <input type="date" id="followUpDate" name="dateofprescription"
                            value="<?php echo $_POST['CurrentDate'] ?? ''; ?>">
                    </div>

                    <center>
                        <div class="form-field">
                            <button class="btn">View</button>
                            <a href="./M_HealthInfo.php"><button class="w-20 btn btn-success btn-lg my-3"
            ...
            if (isset($result) && !empty($result)) {
                ...
                <table border="1">
                    <tr>
                        ...
                        <th style="font-weight: bold;">Beneficiary UID</th>
                        <th style="font-weight: bold;">Operator UID</th>
