...
<body>
        ...
        <div class="form-container">
            <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
                ...
                <center>
                    ...
                    <a href="./M_HealthInfo.php"><button class="w-20 btn btn-success btn-lg my-3" type="submit">Go back</button></a>
                </center>
            </form>
        </div>
    </section>
    ...
    <?php
        if (($_SERVER["REQUEST_METHOD"] == "POST") && isset($_POST['hid']) && isset($_POST['ouid']) && isset($_POST['issuedate']) && isset($_POST['buid']) && !empty($_POST['hid']) && !empty($_POST['ouid']) && !empty($_POST['issuedate']) && !empty($_POST['buid'])) {
            ...
            $tempSql = "SELECT * FROM hms_project_database.prescription WHERE BUID = $buid AND CurrentDate = '$prescriptiondate' AND OUID = $ouid AND HID = $hid;";
        ...
        if (($_SERVER["REQUEST_METHOD"] == "POST") && isset($_POST['remarks']) && isset($_POST['followup']) && isset($_POST['allergies']) && isset($_POST['height']) && isset($_POST['weight']) && isset($_POST['medicines'])) {
            ...
            if (!empty($buid) && !empty($ouid) && !empty($hid) && !empty($prescriptiondate)) {
                $sql1 = "UPDATE hms_project_database.prescription SET Remarks = '$remarks', FollowUp = '$followup', Medicines = '$medicines' WHERE BUID = $buid AND OUID = $ouid AND HID = $hid AND CurrentDate = '$prescriptiondate';";

                $sql2 = "UPDATE hms_project_database.clientinfo SET AllergiesReactions = '$allergies', Height = $height, Weight = $weight, BMI = $bmi WHERE UID = $buid;";
