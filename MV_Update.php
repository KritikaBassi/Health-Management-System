...
<body>
        ...
        <center><h1>Vaccination Record:</h1></center>
            ...
            <div class="form-container">
                <div class="form-field">
                    ...
                    <input type="number" value="<?php echo $_POST['buid'] ?? ''; ?>" id="BUID" name="buid" required=""
                        maxlength="12"
                ...
                <div class="form-field">
                    ...
                    <input type="number" value="<?php echo $_POST['ouid'] ?? ''; ?>" id="OUID" name="ouid" required=""
                        maxlength="12"
                        oninput="javascript: if (this.value.length > this.maxLength) this.value = this.value.slice(0, this.maxLength);">
                </div>
                ...
                <div class="form-field">
                    ...
                    <input type="number" id="HID" value="<?php echo $_POST['hid'] ?? ''; ?>" name="hid" required=""
                        maxlength="12"
                        oninput="javascript: if (this.value.length > this.maxLength) this.value = this.value.slice(0, this.maxLength);">
                </div>
                ...
                <div class="form-field">
                    ...
                    <input type="date" id="followUpDate" value="<?php echo $_POST['mydate'] ?? ''; ?>" name="mydate"
                        required="">
                </div>

                <div class="form-field">
                    <label for="Vaccination">Vaccination For:</label>
                    <select class="form-select" id="Vaccination" name="vaccine" required=""
                        value="<?php echo $_POST['vaccine'] ?? ''; ?>">
            ...
            <div id="data_div" style="display: none;">
                <form method="post" action="<?php echo htmlspecialchars($_SERVER[" PHP_SELF"]); ?>">
                    ...
                    <div class="form-container">
                        ...
                        <input type="radio" id="vaccinated_yes" name="status" value="Yes"
                            onclick="showReason(false);showRest(true)"> Yes
                        <input type="radio" id="vaccinated_no" name="status" value="No"
                            onclick="showReason(true);showRest(false)"> No

                        <div id="reason_div" style="display: none;">
                            <label for="reason">Reason:</label>
                            <input type="text" id="reason" name="reason" rows="4" cols="80"
                                placeholder="Enter reason for not being vaccinated"><br>
                        </div>
                        ...
                        <div id="rest_div" style="display: none;">
                            ...
                            <div class="form-field">
                                ...
                                <input type="number" id="DoseNo" name="dose" maxlength="1"
                                    oninput="javascript: if (this.value.length > this.maxLength) this.value = this.value.slice(0, this.maxLength);">
                            </div>
                        ...
                        <center>
                            <button class="btn">Submit</button>
                            <a href="./M_HealthInfo.php"><button class="btn btn-success">Go back</button></a>
                        </center>
                    </div>
                </form>
            </div>
    ...
    if (($_SERVER["REQUEST_METHOD"] == "POST") && isset($_POST['buid']) && isset($_POST['vaccine']) && isset($_POST['hid']) && isset($_POST['mydate']) && isset($_POST['ouid']) && ($_POST['vaccine'] != "")) {
        ...
        $tempSql = "SELECT * FROM hms_project_database.vaccines WHERE BUID = $buid AND Date = '$mydate' AND OUID = $ouid AND HID = $hid AND Vaccine = '$vaccine';";
    ...
    if (($_SERVER["REQUEST_METHOD"] == "POST") && isset($_POST['status'])) {
        ...
        if (isset($_POST['name']) && isset($_POST['dose']) && !empty($_POST['dose']) && !empty($_POST['name'])) {
            ...
            $sql = "UPDATE hms_project_database.vaccines SET Status = '$status', Name = '$name', Reason = NULL, Dose = $dose WHERE BUID = $buid AND Date = '$mydate' AND OUID = $ouid AND HID = $hid AND Vaccine = '$vaccine';";
        } else {
            $reason = test_input($_POST['reason']);
            $sql = "UPDATE hms_project_database.vaccines SET Status = '$status', Name = NULL, Dose = 0, Reason = '$reason' WHERE BUID = $buid AND Date = '$mydate' AND OUID = $ouid AND HID = $hid AND Vaccine = '$vaccine';";
        }
