...
<body>
    <div class="container">
        ...
        <div id="data_div" style="display: none;">
            ...
            <form id="medicalForm" method="post" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>">
                ...
                <input type="radio" id="currentStatus" name="status" value="Ongoing"
                    onclick="showDiagnosed(true);showOther(false);showTreated(false)"> Ongoing
                <input type="radio" id="currentStatus" name="status" value="Treated"
                    onclick="showDiagnosed(false);showOther(false);showTreated(true)"> Treated
                <input type="radio" id="currentStatus" name="status" value="Other"
                    onclick="showDiagnosed(true);showOther(true);showTreated(true)"> Other
                <br>
                <br>
    ...
    if (($_SERVER["REQUEST_METHOD"] == "POST") && isset($_POST['buid']) && isset($_POST['disease'])) {
        ...
        $tempSql = "SELECT * FROM hms_project_database.medical WHERE BUID = $buid AND Disease = '$disease';";
    ...
    <?php
                ...
                if (($_SERVER["REQUEST_METHOD"] == "POST") && isset($_POST['tsubmit'])) {
                    ...
                    if (isset($_POST['status']) && !empty($_POST['status'])) {
                        ...
                        $sql = "UPDATE hms_project_database.medical SET Status = '$status' WHERE BUID = $buid AND Disease = '$disease';";
                        addDb($conn, $sql);
                    } else {
                        $sql = "UPDATE hms_project_database.medical SET Status = NULL WHERE BUID = $buid AND Disease = '$disease';";
                        addDb($conn, $sql);
                    }

                    if (isset($_POST['treated']) && !empty($_POST['treated'])) {
                        $treated = test_input($_POST['treated']);
                        $sql = "UPDATE hms_project_database.medical SET Treated = '$treated' WHERE BUID = $buid AND Disease = '$disease';";
                        addDb($conn, $sql);
                    } else {
                        $sql = "UPDATE hms_project_database.medical SET Treated = NULL WHERE BUID = $buid AND Disease = '$disease';";
                        addDb($conn, $sql);
                    }

                    if (isset($_POST['diagnosed']) && !empty($_POST['diagnosed'])) {
                        $diagnosed = test_input($_POST['diagnosed']);
                        $sql = "UPDATE hms_project_database.medical SET Diagnosed = '$diagnosed' WHERE BUID = $buid AND Disease = '$disease';";
                        addDb($conn, $sql);
                    } else {
                        $sql = "UPDATE hms_project_database.medical SET Diagnosed = NULL WHERE BUID = $buid AND Disease = '$disease';";
                        addDb($conn, $sql);
                    }

                    if (isset($_POST['other']) && !empty($_POST['other'])) {
                        $other = test_input($_POST['other']);
                        $sql = "UPDATE hms_project_database.medical SET Other = '$other' WHERE BUID = $buid AND Disease = '$disease';";
                        addDb($conn, $sql);
                    } else {
                        $sql = "UPDATE hms_project_database.medical SET Other = NULL WHERE BUID = $buid AND Disease = '$disease';";
                        addDb($conn, $sql);
                    }

                    if (isset($_POST['stage']) && !empty($_POST['stage'])) {
                        $stage = test_input($_POST['stage']);
                        $sql = "UPDATE hms_project_database.medical SET Stage = '$stage' WHERE BUID = $buid AND Disease = '$disease';";
                        addDb($conn, $sql);
                    } else {
                        $sql = "UPDATE hms_project_database.medical SET Stage = NULL WHERE BUID = $buid AND Disease = '$disease';";
                        addDb($conn, $sql);
                    }
                    if (isset($_POST['medicines']) && !empty($_POST['medicines'])) {
                        $medicines = test_input($_POST['medicines']);
                        $sql = "UPDATE hms_project_database.medical SET Medicines = '$medicines' WHERE BUID = $buid AND Disease = '$disease';";
                        addDb($conn, $sql);
                    } else {
                        $sql = "UPDATE hms_project_database.medical SET Medicines = NULL WHERE BUID = $buid AND Disease = '$disease';";
                        addDb($conn, $sql);
                    }
                    if (isset($_POST['family']) && !empty($_POST['family'])) {
                        $family = test_input($_POST['family']);
                        $sql = "UPDATE hms_project_database.medical SET Family = '$family' WHERE BUID = $buid AND Disease = '$disease';";
                        addDb($conn, $sql);
                    } else {
                        $sql = "UPDATE hms_project_database.medical SET Family = NULL WHERE BUID = $buid AND Disease = '$disease';";
                        addDb($conn, $sql);
                    }
                    if (isset($_POST['findings']) && !empty($_POST['findings'])) {
                        $findings = test_input($_POST['findings']);
                        $sql = "UPDATE hms_project_database.medical SET Findings = '$findings' WHERE BUID = $buid AND Disease = '$disease';";
                        addDb($conn, $sql);
                    } else {
                        $sql = "UPDATE hms_project_database.medical SET Findings = NULL WHERE BUID = $buid AND Disease = '$disease';";
                        addDb($conn, $sql);
                    }
