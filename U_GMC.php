<?php
    ...
    $sql = "SELECT * FROM hms_project_database.clientinfo WHERE UID = $uid;";
    ...
    $sql = "SELECT * FROM hms_project_database.prescription WHERE BUID = $uid ORDER BY CurrentDate DESC;";
    ...
    $sql = "SELECT * FROM hms_project_database.medical WHERE BUID = $uid ORDER BY ReportGenerated DESC;";
    if ((mysqli_num_rows(mysqli_query($conn, $sql))) > 0) {

    }

    // RETRIEVING VACCINATION HISTORY INFORMATION
    $sql = "SELECT Vaccine, Dose FROM hms_project_database.vaccines AS V WHERE BUID = $uid AND Dose = (SELECT MAX(Dose) FROM hms_project_database.vaccines WHERE Vaccine = V.Vaccine);";
    if ((mysqli_num_rows(mysqli_query($conn, $sql))) > 0) {
