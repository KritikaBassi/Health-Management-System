...
<body>
    ...
    if (($_SERVER["REQUEST_METHOD"] == "POST") && isset($_POST['buid']) && isset($_POST['ouid']) && isset($_POST['hid']) && isset($_POST['sampleCollectionTime'])) {
        ...
        $tempSql = "SELECT * FROM hms_project_database.invxray WHERE BUID = $buid AND OUID = $ouid AND HID = $hid AND DateOfTest = '$sampleCollectionTime';";
    ...
    ?><script>
                ...
                if (($_SERVER["REQUEST_METHOD"] == "POST") && isset($_POST['tUpdate'])) {
                    ...
                    if (isset($_POST['modality']) && !empty($_POST['modality']) && ($_POST['modality'] != null)) {
                        ...
                        $sql = "UPDATE hms_project_database.invxray SET Modality = '$modality' WHERE BUID = $buid AND OUID = $ouid AND HID = $hid AND DateOfTest = '$sampleCollectionTime';";
                        addData($conn, $sql);
                    }
                    ...
                    if (isset($_FILES['image']) && ($_FILES['image']['error'] == 0)) {
                        ...
                        if ($output != "Image converted successfully!") {
                            ...
                            $sql = "UPDATE hms_project_database.invxray SET Image = '$image' WHERE BUID = $buid AND OUID = $ouid AND HID = $hid AND DateOfTest = '$sampleCollectionTime';";
                            addData($conn, $sql);
                        }
                    }
                    ...
                    if (isset($_POST['part']) && !empty($_POST['part']) && ($_POST['part'] != null)) {
                        ...
                        $sql = "UPDATE hms_project_database.invxray SET Part = '$part' WHERE BUID = $buid AND OUID = $ouid AND HID = $hid AND DateOfTest = '$sampleCollectionTime';";
                        addData($conn, $sql);
                    }

                    if (isset($_POST['technique']) && !empty($_POST['technique']) && ($_POST['technique'] != null)) {
                        $technique = test_input($_POST['technique']);
                        $sql = "UPDATE hms_project_database.invxray SET Technique = '$technique' WHERE BUID = $buid AND OUID = $ouid AND HID = $hid AND DateOfTest = '$sampleCollectionTime';";
                        addData($conn, $sql);
                    }

                    if (isset($_POST['conclusion']) && !empty($_POST['conclusion']) && ($_POST['conclusion'] != null)) {
                        $conclusion = test_input($_POST['conclusion']);
                        $sql = "UPDATE hms_project_database.invxray SET Conclusion = '$conclusion' WHERE BUID = $buid AND OUID = $ouid AND HID = $hid AND DateOfTest = '$sampleCollectionTime';";
                        addData($conn, $sql);
                    }

                    if (isset($_POST['impression']) && !empty($_POST['impression']) && ($_POST['impression'] != null)) {
                        $impression = test_input($_POST['impression']);
                        $sql = "UPDATE hms_project_database.invxray SET Impression = '$impression' WHERE BUID = $buid AND OUID = $ouid AND HID = $hid AND DateOfTest = '$sampleCollectionTime';";
                        addData($conn, $sql);
                    }

                    if (isset($_POST['findings']) && !empty($_POST['findings']) && ($_POST['findings'] != null)) {
                        $findings = test_input($_POST['findings']);
                        $sql = "UPDATE hms_project_database.invxray SET Findings = '$findings' WHERE BUID = $buid AND OUID = $ouid AND HID = $hid AND DateOfTest = '$sampleCollectionTime';";
                        addData($conn, $sql);
                    }
